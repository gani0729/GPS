...
class Displapy(gym.Env):
    def __init__(self, order_data, cut_data, setup_time, device_max_1, device_max_2):
        ...
        self.action_space = spaces.Discrete(3)  # SETUP1, SETUP2, SETUP1(50%), SETUP2(50%),  STAY
        self.observation_space = spaces.MultiDiscrete([210, 4, self.device_max, 2, 7000])
...
#        if (p_amount_set_up*6) >= required_mol and p_device == device and p_amount_set_up != amount_set_up:
#            reward -= 1000
#            info['abandon_amount'] = p_amount_set_up
        if p_amount_set_up == device_max_1 or p_amount_set_up == device_max_2:
            if p_device == device and p_amount_set_up == amount_set_up and action != 2:
                reward = -10000
                info['abandon_amount'] = p_amount_set_up
        ...
        if (p_amount_set_up*6) >= required_mol and p_amount_set_up != amount_set_up:
            reward = -7000
            info['abandon_amount'] = p_amount_set_up
        elif p_amount_set_up > 0 and (p_amount_set_up * 6) < required_mol:
