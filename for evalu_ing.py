...
def evaluate_policy_Display(
    ...
    epi_rewards_undiscounted, epi_rewards_discounted, epi_success_moves, epi_abandons, episode_lengths = [], [], [], [], []
    epi_rewards_undiscounted, epi_rewards_discounted, epi_success_moves, epi_abandons, episode_lengths = [], [], [], [], []
    for i in range(n_eval_episodes):
        ...
        while not done:
            ...
            episode_length = float(_info[0]['use_time'] /24)
        ...
        epi_abandons.append(epi_abandon)
        episode_lengths.append(episode_length)
    mean_discounted_reward = np.mean(epi_rewards_discounted)
    std_discounted_reward = np.std(epi_rewards_discounted)
    if return_episode_rewards:
        return epi_rewards_undiscounted, epi_rewards_discounted, epi_success_moves, epi_abandons, episode_lengths
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.log_path is not None:
                ...
                self.results_abandon.append(epi_abandons)
            ...
            mean_abandon, std_abandon = np.mean(epi_abandons), np.std(epi_abandons)
...
class Display(gym.Env):
    ...
    def excess_penalty2(self, device, c_time, demand):
        ...
        penalty3 = penalty_mol1 + penalty_mol2 + penalty_mol3 + penalty_mol4
    ...
    def step(self, action):
        ...
        if p_amount_set_up > 0 and action != 2:
            ...
            if p_device == 0:
                if p_amount_set_up > (device_max_1 * 0.2):
                    reward -= 0.576 * 10000
                    info['Failed_use'] = True
            elif p_device == 1:
                if p_amount_set_up > (device_max_2 * 0.2):
                    reward -= 0.576 * 10000
                    info['Failed_use'] = True
...
if __name__ == '__main__':
    ...
    order_data_orig = pd.read_csv('C:/Users/admin/Desktop/order.csv')
    order_data = copy.deepcopy(order_data_orig)
    cut_data = pd.read_csv('C:/Users/admin/Desktop/cut_yield.csv')
    setup_time = 28
    device_max_1 = 150
    device_max_2 = 130
    ...
    # eval_env = Display(order_data=order_data, cut_data=cut_data, setup_time=setup_time, device_max_1=device_max_1, device_max_2=device_max_2)
    # #
    # cb = EvalCallback_Display(eval_env=eval_env, n_eval_episodes=10, eval_freq=10000,
    #                           log_path="./model",
    #                           best_model_save_path="./best_model"
    #                           )
    model = PPO2('MlpPolicy', env, verbose=1)
    total_timesteps = int(2.0e4)
