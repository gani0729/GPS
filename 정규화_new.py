...
class Displapy(gym.Env):
    def __init__(self, order_data, cut_data, setup_time, device_max_1, device_max_2):
       ...
       ## observation_space 나타내기 위한 max 함수들
        ...
        #self.observation_space = spaces.Box(low=np.array([0, 1, 0, 0, 0]), high=np.array([1, 4, 1, 2, 10000]),
                                            #dtype=np.float32)
        #self.observation_space.high[-1] = 10000.0
        ...
        # state = required_mol, mol_name, amount_set_up, device, c_time
        ...
        # "time" 열을 제외한 열들만 선택
        self.columns_to_normalize = self.demand.columns.difference(['time'])
        self.demand_to_normalize = self.demand[self.columns_to_normalize]

        # device_max열 추가
        self.demand_to_normalize['max_device_1'] = [device_max_1 * 6] * len(self.demand_to_normalize)
        self.demand_to_normalize['max_device_2'] = [device_max_2 * 6] * len(self.demand_to_normalize)

        #정규화 공식을 이용한 정규화함수
        def normalize_dataframe(df):
            min_value = 0
            max_value = self.device_max * 6
            normalized_df = (df - min_value) / (max_value - min_value)
            return normalized_df

        # 데이터프레임 정규화 수행
        self.normalized_df = normalize_dataframe(self.demand_to_normalize)
        self.demand = pd.concat([self.demand['time'], self.normalized_df], axis=1)
    ...
    def step(self, action):
        ...
        #set-up action
        #amount_set_up은 mol 양
        if action == 0: #소자1을 셋업
            amount_set_up = self.demand['max_device_1'][0]
            device = 0
            c_time += self.setup_time
            self.setup_times.append(self.setup_time)
        elif action == 1: #소자2를 셋업
            amount_set_up = self.demand['max_device_2'][0]
            device = 1
            c_time += self.setup_time
            self.setup_times.append(self.setup_time)
        ...
        if p_amount_set_up < 0.66666667 and p_amount_set_up != amount_set_up:
            reward -= 500
        ...
        #소자 태우는 상태 전이 표현
        #시간은 정규화된 값이 아닌 원래의 mol양을 가지고 시간으로 반환
        if required_mol < possible_mol and required_mol > 0:
            self.running_times.append((required_mol * (max((device_max_1*6),(device_max_2*6)) - 0) + 0)/ 6)
            if (required_mol % 6).any() == 0:
                c_time += (required_mol * (max((device_max_1*6),(device_max_2*6)) - 0) + 0) // 6
            else:
                c_time += ((required_mol * (max((device_max_1 * 6), (device_max_2 * 6)) - 0) + 0) // 6) + 1
            ...
            self.running_times.append(((amount_set_up * (max((device_max_1 * 6), (device_max_2 * 6)) - 0) + 0)/6))
            if (required_mol % 6).any() == 0:
                c_time += (required_mol * (max((device_max_1 * 6), (device_max_2 * 6)) - 0) + 0) // 6
            else:
                c_time += ((required_mol * (max((device_max_1 * 6), (device_max_2 * 6)) - 0) + 0) // 6) + 1
