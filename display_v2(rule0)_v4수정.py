...
class Displapy(gym.Env):
    def __init__(self, order_data, cut_data, setup_time, device_max_1, device_max_2):
       ...
       ## observation_space 나타내기 위한 max 함수들
        ...
        # 하나의 mol 성형이 끝났을 때
        self.reward_per_success = 20

        # 하나의 mol을 완전히 성형하진 못했지만, 투입은 해놓은 상태라면
        self.reward_per_success2 = 10

        # 세팅한 소자를 모두 사용했을 때
        self.reward_operation_max = 30
    ...
    def excess_penalty(self, due_time, c_time, demand): #현재 성형해야 하는 mol의 납기가 지났을 때, 패널티
        penalty1 = 0
        coef1 = 0.9
    ...
    #def excess_penalty(self,demand,c_time):
     #   penalty1 = 0
      #  coef1 = 0.5
       # time1 = demand.loc[first_nonzero_1, 'time']
        #time2 = demand.loc[first_nonzero_2, 'time']
        #time3 = demand.loc[first_nonzero_3, 'time']
        #time4 = demand.loc[first_nonzero_4, 'time']
        #diff1 = c_time - time1
        #diff2 = c_time - time2
        #diff3 = c_time - time3
        #diff4 = c_time - time4
       # penalty1 += coef1 * (diff1**2)+ coef1*(diff2**2) + coef1*(diff3**2) + coef1*(diff4**2)
       # return penalty1
    ...
    def step(self, action):
        ...
        # if p_amount_set_up > 100 and p_amount_set_up > 0 and p_amount_set_up != amount_set_up:
        #     reward -= 500
        ...
        #소자 태우는 상태 전이 표현
        if required_mol < possible_mol and required_mol > 0 and possible_mol > 0:
        ...
        if required_mol > 0 and possible_mol == 0:
            reward -= 5
        ...
        #first_nonzero_1 = None
        #first_nonzero_2 = None
        #first_nonzero_3 = None
        #first_nonzero_4 = None
        for i in range(len(self.demand)):
            if self.demand.loc[i, 'MOL_1'] != 0:
                first_nonzero_row_1 = i
                break
                due_time1 = 0
                if first_nonzero_row_1 is not None:
                    due_time1 = self.demand.loc[first_nonzero_row_1, 'time']
        for j in range(len(self.demand)):
            if self.demand.loc[j, 'MOL_2'] != 0:
                first_nonzero_row_2 = j
                break
                due_time2=0
                if first_nonzero_row_2 is not None:
                    due_time2 = self.demand.loc[first_nonzero_row_2, 'time']
        #due_time2 = self.demand.loc[first_nonzero_row_2, 'time']
        for k in range(len(self.demand)):
            if self.demand.loc[k, 'MOL_3'] != 0:
                first_nonzero_row_3 = k
                break
                due_time3=0
                if first_nonzero_row_3 is not None:
                    due_time3 = self.demand.loc[first_nonzero_row_3, 'time']
        #due_time3 = self.demand.loc[first_nonzero_row_3, 'time']
        for l in range(len(self.demand)):
            if self.demand.loc[l, 'MOL_4'] != 0:
                first_nonzero_row_4 = l
                break
                due_time4=0
                if first_nonzero_row_4 is not None:
                    due_time4 = self.demand.loc[first_nonzero_row_4, 'time']
        #due_time4 = self.demand.loc[first_nonzero_row_4, 'time']


        #due_time1 = self.demand.loc[first_nonzero_1, 'time']
        #due_time2 = self.demand.loc[first_nonzero_2, 'time']
        #due_time3 = self.demand.loc[first_nonzero_3, 'time']
        #due_time4 = self.demand.loc[first_nonzero_4, 'time']
        ...
        # month, day, c_time = self.plustime(month, day, c_time)
        if int(due_time1) != 0 or int(due_time2) != 0 or int(due_time3) != 0 or int(due_time4) != 0:
            if (due_time1 < c_time).any():
                reward -= self.excess_penalty(due_time1, c_time)
                info['excess'] = True
            elif (due_time2 < c_time).any():
                reward -= self.excess_penalty(due_time2, c_time)
                info['excess'] = True
            elif (due_time3 < c_time).any():
                reward -= self.excess_penalty(due_time3, c_time)
                info['excess'] = True
            elif (due_time4 < c_time).any():
                reward -= self.excess_penalty(due_time4, c_time)
                info['excess'] = True
...
if __name__ == '__main__':
    ...
    model = PPO2('MlpPolicy', env, learning_rate = 1.0e-4, verbose=1) # 2.5e-2, 1.0e-2, 2.5e-3, 1.0e-3, 2.5e-4, 1.0e-4
    model.learn(total_timesteps=int(5.0e4))
