...
class Displapy(gym.Env):
    def __init__(self, order_data, cut_data, setup_time, device_max_1, device_max_2):
       ...
       ## observation_space 나타내기 위한 max 함수들
        ...
        # 하나의 mol 성형이 끝났을 때
        self.reward_per_success = 500

        # 세팅한 소자를 모두 사용했을 때
        self.reward_operation_max = 100.0
    ...
    def step(self, action):
        ...
        #소자 태우는 상태 전이 표현
        if (required_mol < possible_mol).any() and required_mol > 0:
        ...
        elif (required_mol == possible_mol).any() and required_mol > 0:
            # self.demand = self.update_model(self.demand, order_info, required_mol)
...
if __name__ == '__main__':
    ...
    # model = PPO2('MlpPolicy', env, learning_rate = 1.0e-4, verbose=1)
